...
from eph_mapf.src.data.generate_data import generate_movingai
...
def get_num_agents(args, mapname):
    if args.agentNums == "increment":
        ...
        num_agents_list = [int(x) for x in args.agentNums.split(",")]
    return num_agents_list

def simulate(model_name, mapnames, agent_count, shield_type, lacam_lookahead, args):
    """Inputs:
        ...
        goal_locations: (N,2)
    """
    config.ensemble = [(False, None, None, False,None),] #TODO idk what this means

    config.test_folder = "./eph_mapf/test_set"
    config.save_map_config = True
    config.save_positions = True
    config.data_mode = "movingai"
    config.test_timeout = args.timeLimit
    
    
    agent_nums = [get_num_agents(args, m) for m in mapnames]
    map_agent_list = []
    for i, agent_num_for_map in enumerate(agent_nums):
        for agent_num in agent_num_for_map:
            map_agent_list.append((mapnames[i],agent_num))

    # pdb.set_trace()
    # config.action_dim=2
    config.test_env_settings = tuple(map_agent_list)
    ...
    def update_csv(mapName, agentNum, success, total_cost_true, runtime):
        csv_dict['mapName'].append(mapName)
        csv_dict['agentNum'].append(agentNum)
        csv_dict['success'].append(success)
        csv_dict['total_cost_true'].append(total_cost_true)
        csv_dict['runtime'].append(runtime)


    results = test_model('65000', config=config) # change to model_name if we train others
    for key in results:
        map_name, num_agents = key[0], key[1]
        val_dict = results[key]
        successes, costs, runtimes = val_dict['success'], val_dict['steps'], val_dict['runtime']
        success_rate, average_cost, average_runtime = np.mean(successes), np.mean(costs), np.mean(runtimes)
        update_csv(map_name, num_agents, success_rate, average_cost, average_runtime)
    
    df = pd.DataFrame.from_dict(csv_dict)
    df.to_csv(args.outputCSVFile)
...
def main(args: argparse.ArgumentParser):
    ...
    print(mapname)
    simulate(args.modelPath, mapname, args.agentNums, args.shieldType, args.lacamLookahead, args)
    endtime = time.time()
    ...
    #         writer.writerow(['mapName', 'scenFile', 'agentNum', 'shieldType', 'lacamLookahead',
    #                          'modelPath', 'useGPU', 
    #                          'success', 'total_cost_true', 
    ...
    #                      args.modelPath, args.useGPU, 
    #                      success, toƒtal_cost_true,  num_agents_at_goal, runtime])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--agentNums', type=str, required=True)
