...
class CommLayer(nn.Module):
    ...
    def __init__(
        self, input_dim=256, message_dim=32, pos_embed_dim=16, num_heads=4
    ...
    def forward(self, hidden, relative_pos, comm_mask):
        ...
        message_q = self.hidden_query(hidden).view(
        ...
        # attention
        attn_score = (message_q * message_k).sum(
            4, keepdim=True
        ) / self.message_dim**0.5  # batch_size x num_agents x num_agents x self.num_heads x 1
        ...
        agg_message = self.head_agg(
            agg_message
        )  # batch_size x num_agents x self.num_heads*self.message_dim
        ...
        # Feed forward
        output2 = self.feed_forward(
            self.norm2(output1)
        )  # batch_size*num_agents x hidden_dim
