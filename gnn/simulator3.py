...
DIRECTION_LABELS = np.array([(0,0), (0,1), (1,0), (-1,0), (0,-1)]) # for multistep input
...
def lacam(start_locations, goal_locations, bd, grid_map, getActionPrefsFromLocs, multi_inputs,
    ...
    class HLNode:
        def __init__(self, state, action_preferences, parent, multi_inputs) -> None:
            self.state = state
            ...
            self.multi_inputs = multi_inputs # (N,h,5)
    ...
    curNode = HLNode(start_locations, getActionPrefsFromLocs(start_locations, multi_inputs), None, multi_inputs)
    ...
    while len(mainStack) > 0:
        ...
        new_locs, new_multi_inputs = curNode.getNextState()
        if new_locs is None:
            continue
        numNodesExpanded += 1
        ...
        if np.all(np.equal(new_locs, goal_locations)):
            print(f"Stopping as found goal in LaCAM, depth: {curNode.depth}")
            success = True
            break
        ...
        if key in stateToHLNodes.keys(): # Already visited/created this state
            ...
            # Create a new HLNode
            newHLNode = HLNode(new_locs, getActionPrefsFromLocs(new_locs, new_multi_inputs), curNode, new_multi_inputs)
            numGenerated += 1
    ...
    for step in tqdm(range(max_steps)):
        ...
        if shield_type in ["CS-PIBT", "CS-Freeze"]:
            ...
            next_locs, lacamFoundSolution, numNodesExpanded, numGenerated = lacam(cur_locs, goal_locations, 
                                                    bd, grid_map, getActionPrefsFromLocs, multi_inputs,
                                                    scaled_lookahead, start_time, args.timeLimit)
            ...
            if lacamFoundSolution:
                ...
                print(f"LaCAM found solution at step: {step}")
        ...
        # pdb.set_trace()
        multi_inputs = update_multi_inputs(multi_inputs, new_move)
...
def update_multi_inputs(multi_inputs, new_move):
    if multi_inputs.shape[1] == 0:
        return multi_inputs
    new_onehot = np.all(new_move[:,None,:]==DIRECTION_LABELS, axis=2)[:,None,:] # (N,1,5)
    return np.concatenate((multi_inputs[:,1:,:], new_onehot), axis=1) # (N,h,5)
...
def main(args: argparse.ArgumentParser):
    ...
    if not os.path.exists(args.scenFile):
        raise FileNotFoundError(f'Scen file: {args.scenFile} not found.')
    ...
    # Load the bd
    bdNpzFile = f"{args.bdPath}/{args.mapName}_bds.npz"
    scenToGoalsNpzFile = f"{args.bdPath}/{args.mapName}_goals.npz"
    if not os.path.exists(bdNpzFile):
    ...
    scen_to_goals_npz = np.load(scenToGoalsNpzFile) # scen to goals: dict[scenname] = goal indices (N,) 
    bd_npz = np.load(bdNpzFile)['arr_0'] # bd_npz[goal_index] = bd (W,H)

    if bd_key not in scen_to_goals_npz:
        raise ValueError(f'BD key {bd_key} not found in the bd file')
    ...
    if not os.path.exists(args.modelPath):
        raise FileNotFoundError(f'Model file: {args.modelPath} not found.')
    ...
    print(f"Success: {success}, Total cost true: {total_cost_true}, Total cost not at goal: {total_cost_not_resting_at_goal}, "+
            f"Num agents at goal: {num_agents_at_goal}/{num_agents}, Seconds spent: {total_simulate_time}")
    ...
    if not os.path.exists(args.outputCSVFile):
        ...
        os.makedirs(os.path.dirname(args.outputCSVFile), exist_ok=True)
    ...
    os.makedirs(os.path.dirname(args.outputPathsFile), exist_ok=True)
    ...
    for t in sampled_timesteps:
        ...
        mapname, scenname, _ = getMapScenAgents(args.scenFile)
        custom_scenname = f"{scenname}_t{t}"
        prefix = os.path.dirname(args.outputPathsFile)
        scenFilepath = f"{prefix}/{mapname}.{custom_scenname}.{num_agents}.scen"
        # pdb.set_trace()
