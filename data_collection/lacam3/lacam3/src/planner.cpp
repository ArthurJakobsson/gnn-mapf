...
Solution Planner::solve()
{
  ...
  SolutionOrdering plan_ordering = backtrackOrdering(H_goal);
  for (auto p : EXPLORED) delete p.second;  // memory management
  return solution;
}

SolutionOrdering backtrackOrdering(HNode *H) {
  SolutionOrdering plan;
  auto _H = H;
  while (_H != nullptr) {
    plan.push_back(_H->order);
    _H = _H->parent;
  }
...
Solution Planner::get_refined_plan(const Solution &plan)
{
  ...
  if (depth < 1 && plan.size() > 3 &&
    ...
    std::tuple<Solution, SolutionOrdering> res = planner_tmp.solve();
    info(4, verbose, deadline, "refiner-", planner_tmp.seed,
