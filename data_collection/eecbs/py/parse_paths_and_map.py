...
class PipelineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if abs(dijk[self.k, self.k]) >= 10000: pdb.set_trace()
        if grid[self.k, self.k] != 0: pdb.set_trace()
        self.best_middle = max(self.best_middle, abs(dijk[self.k, self.k]))
        # dijk = np.where(dijk == 1073741823, 0, dijk) # TODO np.where after you subtract (first subract, then np.where(bignumber-center, filler_value))

        dijk = dijk - dijk[self.k,self.k]
        # pdb.set_trace()
        dijk *= (1 - grid)
        # dijk = np.where(dijk > 1000000000, 0, dijk) # TODO np.where after you subtract (first subract, then np.where(bignumber-center, filler_value))
...
def parse_map(mapfile):
    ...
    '''
    # if "final" in mapfile: pdb.set_trace()
...
def main():
    ...
    loader = PipelineDataset(trainOut + ".npz", 4, float('inf'), 500)
    ...
    pdb.set_trace()
    loader = PipelineDataset(valOut + ".npz", 4, float('inf'), 500)
